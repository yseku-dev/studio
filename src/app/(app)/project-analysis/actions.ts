...
export interface AnalyzeProjectPayload {
  projectFileContent?: string; // Raw text content of the file
  projectFileName?: string;
  projectFileType?: string; // MIME type
  gitUrl?: string;
  analysisPreferences?: string;
  configSource: string; // "global", "agent:id", "workgroup:id"
  // LLMOptions should be resolved on the server based on configSource and localStorageSnapshot
  // So, not directly part of the payload from client unless it's 'global' or a specific agent.
  // For simplicity, client can send its resolved LLM options if not using workgroup.
  llmOptions?: LLMOptions; 
  agents?: AgentConfig[]; // Needed if configSource is workgroup, passed from client's localStorage
  workgroups?: WorkgroupConfig[]; // Needed if configSource is workgroup
  localStorageSnapshot?: LocalStorageSnapshot; // Client's localStorage state for resolving LLM configs
}
...
const LLM_API_TIMEOUT_MS_PROJ_ANALYSIS = 60000 * 2; // 2 minutes per chunk
...
export async function handleAnalyzeProject(
  ...
  const log = (type: 'INFO' | 'ERROR' | 'DEBUG' | 'DETAIL' | 'WARN', message: string, data?: any) => {
    ...
    console.log(logMsg.replace(/\n/g, ' ')); // Log to server console, replace newlines for cleaner single-line log
